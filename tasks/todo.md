...
- [ ] Slice 2 hardening follow-ups (PR #10 review run, owner: 0xnicholasy). Must land before any device-narrowing feature: stamp `projectId` into each persisted `AgentEvent` so an event is authorized against its own record instead of a live session lookup (protocol type change). Also open: torn journal prefix on a mid-loop write failure (`ftruncateSync` back to the pre-write size), no runtime cap/retention on the session index, `SessionIndex.compact` lacks the try/catch its siblings have, and the four pre-existing auth findings (nonce capacity eviction replay, clock-rollback replay, non-eventual concurrent retry outcomes, state-dir write failures unreported at startup). Detail in `tasks/harden-pr/pr-10/backlog.md`; low-severity items in `deferred.md`.
- [ ] Sign off or reject the two accepted risks in `tasks/harden-pr/pr-10/accepted-risk.md` (stale-lock takeover TOCTOU, command map soft cap) before PR #10 merges.
