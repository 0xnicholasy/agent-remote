...
- [ ] Still open from the PR #10 review run (owner: 0xnicholasy): the four pre-existing auth findings — nonce capacity eviction replay, clock-rollback replay, non-eventual concurrent retry outcomes, state-dir write failures unreported at startup. Detail in `tasks/harden-pr/pr-10/backlog.md`; low-severity items in `deferred.md`. The session index already has the cap and retention the backlog asks for.
